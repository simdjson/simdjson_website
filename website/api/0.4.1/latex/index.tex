An overview of what you need to know to use simdjson, with examples.\hypertarget{index_autotoc_md17}{}\doxysection{Requirements}\label{index_autotoc_md17}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md18}{}\doxysection{Including simdjson}\label{index_autotoc_md18}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md19}{}\doxysection{Using simdjson as a C\+Make dependency}\label{index_autotoc_md19}
...
See \href{https://github.com/simdjson/cmakedemo}{\texttt{ our C\+Make demonstration}}.\hypertarget{index_autotoc_md20}{}\doxysection{The Basics\+: Loading and Parsing J\+S\+O\+N Documents}\label{index_autotoc_md20}
...
For best performance, a {\ttfamily parser} instance should be reused over several files\+: otherwise you will needlessly reallocate memory, an expensive process. It is also possible to avoid entirely memory allocations during parsing when using simdjson.\hypertarget{index_autotoc_md21}{}\doxysection{Using the Parsed J\+S\+ON}\label{index_autotoc_md21}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md22}{}\doxysection{C++11 Support and string\+\_\+view}\label{index_autotoc_md22}
...
When we detect that it is unavailable, we use \href{https://github.com/martinmoene/string-view-lite}{\texttt{ string-\/view-\/lite}} as a substitute. In such cases, we use the type alias {\ttfamily using string\+\_\+view = nonstd\+::string\+\_\+view;} to ~\newline
 offer the same A\+PI, irrespective of the compiler and standard library. The macro {\ttfamily S\+I\+M\+D\+J\+S\+O\+N\+\_\+\+H\+A\+S\+\_\+\+S\+T\+R\+I\+N\+G\+\_\+\+V\+I\+EW} will be {\itshape undefined} to indicate that we emulate {\ttfamily string\+\_\+view}.\hypertarget{index_autotoc_md23}{}\doxysection{C++17 Support}\label{index_autotoc_md23}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{index_autotoc_md24}{}\doxysection{Minifying J\+S\+O\+N strings without parsing}\label{index_autotoc_md24}
...
Though it does not validate the J\+S\+ON input, it will detect when the document ends with an unterminated string. E.\+g., it would refuse to minify the string {\ttfamily "this string is not terminated} because of the missing final quote.\hypertarget{index_autotoc_md25}{}\doxysection{U\+T\+F-\/8 validation (alone)}\label{index_autotoc_md25}
...
Your input string does not need any padding. Any string will do. The {\ttfamily validate\+\_\+utf8} function does not do any memory allocation on the heap, and it does not throw exceptions.\hypertarget{index_autotoc_md26}{}\doxysection{J\+S\+O\+N Pointer}\label{index_autotoc_md26}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md27}{}\doxysection{Error Handling}\label{index_autotoc_md27}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{index_autotoc_md28}{}\doxysubsection{Error Handling Example}\label{index_autotoc_md28}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{index_autotoc_md29}{}\doxysubsection{Exceptions}\label{index_autotoc_md29}
...
When used this way, a {\ttfamily simdjson\+\_\+error} exception will be thrown if an error occurs, preventing the program from continuing if there was an error.\hypertarget{index_autotoc_md30}{}\doxysection{Tree Walking and J\+S\+O\+N Element Types}\label{index_autotoc_md30}
...
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{index_autotoc_md31}{}\doxysection{Newline-\/\+Delimited J\+S\+O\+N (ndjson) and J\+S\+O\+N lines}\label{index_autotoc_md31}
...
Both {\ttfamily load\+\_\+many} and {\ttfamily parse\+\_\+many} take an optional parameter {\ttfamily size\+\_\+t batch\+\_\+size} which defines the window processing size. It is set by default to a large value ({\ttfamily 1000000} corresponding to 1 MB). None of your J\+S\+ON documents should exceed this window size, or else you will get the error {\ttfamily simdjson\+::\+C\+A\+P\+A\+C\+I\+TY}. You cannot set this window size larger than 4 GB\+: you will get the error {\ttfamily simdjson\+::\+C\+A\+P\+A\+C\+I\+TY}. The smaller the window size is, the less memory the function will use. Setting the window size too small (e.\+g., less than 100 kB) may also impact performance negatively. Leaving it to 1 MB is expected to be a good choice, unless you have some larger documents.\hypertarget{index_autotoc_md32}{}\doxysection{Thread Safety}\label{index_autotoc_md32}
We built simdjson with thread safety in mind.

The simdjson library is single-\/threaded except for {\ttfamily parse\+\_\+many} which may use secondary threads under its control when the library is compiled with thread support.
...
The C\+PU detection, which runs the first time parsing is attempted and switches to the fastest parser for your C\+PU, is transparent and thread-\/safe.\hypertarget{index_autotoc_md33}{}\doxysection{Backwards Compatibility}\label{index_autotoc_md33}
